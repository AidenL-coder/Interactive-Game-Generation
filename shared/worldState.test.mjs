...
import { validateWorldState, validateDeltaTurn, applySceneDelta, validateParts } from "./worldState.js";
...
console.log("\n--- validateParts (objects assembled from primitives) ---");
{
  ...
  // than a thrown error — a part at NaN or sized 40 metres silently wrecks a scene.
  // Hence strict validation, hence these. Geometry is generated by its own call, so
  // this is validated independently of the WorldState schema.
  check(
    "valid parts pass",
    validateParts([
      { shape: "box", pos: [0, 0.05, 0], size: [0.6, 0.1, 0.5], color: "#6b5a3a" },
      { shape: "cylinder", pos: [0, 0.55, 0], size: [0.08, 1.0, 0.08], color: "#6b5a3a" },
    ]).valid
  );
  check("missing parts rejected", !validateParts(undefined).valid);
  check("empty parts rejected", !validateParts([]).valid);
  check(
    "unknown shape rejected",
    !validateParts([{ shape: "dodecahedron", pos: [0, 0, 0], size: [1, 1, 1], color: "#fff000" }])
      .valid
  );
  check(
    "non-hex part colour rejected",
    !validateParts([{ shape: "box", pos: [0, 0, 0], size: [1, 1, 1], color: "greenish" }]).valid
  );
  check(
    "lathe without profile rejected",
    !validateParts([{ shape: "lathe", pos: [0, 0, 0], size: [1, 1, 1], color: "#ffffff" }]).valid
  );
  check(
    "lathe with profile passes",
    validateParts([
      {
        shape: "lathe",
        pos: [0, 0, 0],
        size: [1, 1, 1],
        color: "#ffffff",
        profile: [[0.5, 0], [0.35, 2.6], [0.5, 3.0]],
      },
    ]).valid
  );
  check(
    "non-numeric pos rejected",
    !validateParts([{ shape: "box", pos: [0, "high", 0], size: [1, 1, 1], color: "#ffffff" }]).valid
  );
  check(
    "all-zero size rejected",
    !validateParts([{ shape: "box", pos: [0, 0, 0], size: [0, 0, 0], color: "#ffffff" }]).valid
  );
}
